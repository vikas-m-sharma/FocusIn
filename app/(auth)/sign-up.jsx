...
import { registerUser } from '../(auth)/sign-up';
    ...
    const submit = () => {
        dispatch(registerUser(form)).then((action) => {
            if (action.type === 'user/registerUser/fulfilled') {
                ...
                alert(action.payload); // Show error from backend
            }
        });
    };
...
//     setSubmitting(true);
//     try {
//       const response = await axios.post('http://192.168.31.70:5001/api/users/register', {
//         name: form.name,
//         email: form.email,
